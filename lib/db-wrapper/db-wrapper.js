...
function db_wrapper()
{
	...
	this.getLogin= async function (params) 
	{
	  try
	  {
	    ...
	    if(result.length > 0)
	    {
	      ...
	      {
	      	logger.log('Invalid username or passsword');
	...
	this.removeTicket = async function(params)
	{
		try
		{
			await db.delNotification(params);
			///
			await db.removeUploadFile(params);
			///
	...
	/////
	this.getUploadFile=async function(params)
	{
		try
		{
			const result = await db.getUploadFile(params);
			logger.log('Uploaded files received successfully');
			console.log('Uploaded files received successfully');
			return result;
		}
		catch(err)
		{
	...
	/////
	this.uploadFile=async function(params)
	{
		try
		{
			const lastId= await db.getLastId();
			await db.uploadFile({'ticketId':lastId[0].Id,'path':params.path});
		...
		{
			logger.log(`Error uploading files ${err}`);
