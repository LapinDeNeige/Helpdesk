...
function db()
{
    ...
    //
    logger.log(`Connecting to database ${process.env.DB_HOST}:${process.env.DB_PORT}`)
    //
    ...
    {
        return con.query(`INSERT INTO ${tableUpload} (ticketId,Path) VALUES(?,?);`,[data.ticketId,data.path]);
    }
    this.getUploadFile=async function (data) 
    {
        return con.query(`SELECT * FROM ${tableUpload} WHERE ticketId=?;`,[data.ticketId]);
    }
    this.removeUploadFile=async function (id) 
    {
        return con.query(`DELETE FROM ${tableUpload} WHERE ticketId=?`,[id]);
    }
    /////
